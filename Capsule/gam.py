...
class CapsuleNet(nn.Module):
    def __init__(self):
        ...
        self.digit_capsules = CapsuleLayer(num_capsules=NUM_CLASSES, num_route_nodes=1152, in_channels=8,
                                           out_channels=16)
...
if __name__ == "__main__":
    ...
    def on_end_epoch(state):
        ...
        info.append(meter_loss.value()[0])
        info.append(meter_accuracy.value()[0])
        torch.save(model.state_dict(), 'epochs/epoch_0.pt')
        log.append(info)
    ...
    np.savetxt("Dynamic_FsMnist.csv", np.asarray(log), delimiter=",")